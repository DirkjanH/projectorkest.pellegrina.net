...
function KT_transformsPaths($templateUrl, $text) {
	...
	$scriptUrl = KT_getUri();
	preg_match_all('/<(a|img|link|script|form|iframe|embed)([^>]*)>/ims',$text, $matches);
	...
	foreach($matches[1] as $key=>$value) {
		...
		if (isset($mmatch[1])) {
			...
			$text = preg_replace("/src\s*=\s*".preg_quote($mydelim . $mmatch[1] . $mydelim, "/")."/ims", 'src="'.KT_Rel2AbsUrl($scriptUrl, $templateUrl, $mmatch[1]).'"', $text);
		}
	}
	...
	if (preg_match('/\$MXW_relPath\s=\s/i', $text)) {
		...
		foreach($matches[1] as $key=>$value){
			$text = str_replace('$MXW_relPath = \'' . $value . '\';', '$MXW_relPath = \'' . KT_Rel2AbsUrl($scriptUrl, $templateUrl, $value) . '\';', $text);
		}
	}
 ...
 * @return string current page and query string (ex. index.php?mod=return );
 */
 ...
 * @return string return the folder (ex. var/www/html/mysite/admin/ );
 */
